                                            ...
                                            <div class="modal-body" id="sign_up">
                                                <div class="login text-center" id="detail">
                                                    ...
                                                    <div class="con_pro" >
                                                        ...
                                                        <button type="button" class="btn fill_btn" data-dismiss="modal" onclick="$('#model_close_false').val('1');">Close</button>
                                                    </div>
                                                </div>                                                
                                            </div>
                                        </div>
                                    ...
                                    <div class="project_desc2">
                                        ...
                                        <div class="col-sm-3 pro_content2 last_head">
                                            ...
                                            <ul>
                                                ...
                                                <li><a href="javascript:void(0)" class="comment" onclick="show_comments(2,<?php echo $project_requirement['prr_id'] ?>)"><i class="fa fa-comment-o" aria-hidden="true"></i></a> <span><?php echo comment_count($project_requirement['prr_id']) ?></span> </li>                                                
                                                <li class="like" id="like<?php echo $j ?>" <?php if(!$project_requirement['like_count']){ echo 'style="display:inline-block"'; } else { echo 'style="display:none"'; } ?> onclick="like_dislike(1,<?php echo $project_requirement['prr_id'] ?>,<?php if(isset($project_detail[0]['prj_id'])){ echo $project_detail[0]['prj_id']; } ?>,$(this).attr('id'))"><a href="javascript:void(0)"><i class="fa fa-thumbs-o-up" aria-hidden="true"></i></a> <span><?php echo $project_requirement['like_count'] ?></span> </li>
                                                <li id="after_like<?php echo $j ?>" class="after_like" <?php if($project_requirement['like_count']){ echo 'style="display:inline-block"'; } else { echo 'style="display:none"'; } ?> onclick="clear_like_dislike(<?php echo $project_requirement['prr_id'] ?>,<?php if(isset($project_detail[0]['prj_id'])){ echo $project_detail[0]['prj_id']; } ?>,$(this).attr('id'))"><a href="javascript:void(0)"><i class="fa fa-thumbs-up" aria-hidden="true"></i></a> <span><?php echo $project_requirement['like_count'] ?></span> </li>
                                                <li class="dislike" id="dislike<?php echo $j ?>" <?php if(!$project_requirement['dislike_count']){ echo 'style="display:inline-block"'; } else { echo 'style="display:none"'; } ?> onclick="like_dislike(2,<?php echo $project_requirement['prr_id'] ?>,<?php if(isset($project_detail[0]['prj_id'])){ echo $project_detail[0]['prj_id']; } ?>,$(this).attr('id'))"><a href="javascript:void(0)"><i class="fa fa-thumbs-o-down" aria-hidden="true"></i></a> <span><?php echo $project_requirement['dislike_count'] ?></span> </li>                                                
                                                <li id="after_dislike<?php echo $j ?>" class="after_dislike" <?php if($project_requirement['dislike_count']){ echo 'style="display:inline-block"'; } else { echo 'style="display:none"'; } ?> onclick="clear_like_dislike(<?php echo $project_requirement['prr_id'] ?>,<?php if(isset($project_detail[0]['prj_id'])){ echo $project_detail[0]['prj_id']; } ?>,$(this).attr('id'))"><a href="javascript:void(0)"><i class="fa fa-thumbs-down" aria-hidden="true"></i></a> <span><?php echo $project_requirement['dislike_count'] ?></span> </li>                                                
                                                <?php if($project_user_type == 'owner' || $project_requirement['user_id']==$user_id){ ?>                                                
...
<input type="hidden" id="model_close_false" value="0" />
                                                <button type="button" class="btn fill_btn" data-dismiss="modal" onclick="$('#model_close_false').val('1');">Close</button>
                                            </div>
                                        </div>                                                
                                    </div>
    ...
    <?php echo $footer ?>
        <script>
            ...
            $(document).ready(function () {
                ...
                if(!$('#model_close_false').val()){
                   location.reload(true);
                } else {
                   $('#model_close_false').val('');
                }
               })
            });
    ...
    if(ld && requirement_id && project_id){            
        $.ajax({
            ...
            success:function(data){      
                ...
                if(data=='success')
                {                 
                var like_count = $('#'+current_id).parent().find('.like').find('span').text();
                if(like_count < 0) { like_count = 0; }
                var dislike_count = $('#'+current_id).parent().find('.dislike').find('span').text();      
                if(dislike_count < 0) { dislike_count = 0; }
                    if(ld=='1')
                    {                                             
                    	  if(like_count == 0 && dislike_count == 0){ dislike_count = 1; }
                          $('#'+current_id).parent().find('.like').css('display','none');
                          $('#'+current_id).parent().find('.like').find('span').text(+like_count+1);
                          $('#'+current_id).parent().find('.after_dislike').css('display','none');
                          $('#'+current_id).parent().find('.after_dislike').find('span').text(+dislike_count-1);
                          $('#'+current_id).parent().find('.dislike').css('display','inline-block');
                          $('#'+current_id).parent().find('.dislike').find('span').text(+dislike_count-1);
                          $('#'+current_id).parent().find('.after_like').css('display','inline-block');
                          $('#'+current_id).parent().find('.after_like').find('span').text(+like_count+1);
                    }
                    else if(ld=='2')
                    {                     
                    	  if(like_count == 0 && dislike_count == 0){ like_count = 1; }                           
                    	  $('#'+current_id).parent().find('.dislike').css('display','none');
                    	  $('#'+current_id).parent().find('.dislike').find('span').text(+dislike_count+1);
                          $('#'+current_id).parent().find('.after_like').css('display','none');
                          $('#'+current_id).parent().find('.after_like').find('span').text(+like_count-1);
                          $('#'+current_id).parent().find('.like').css('display','inline-block');
                          $('#'+current_id).parent().find('.like').find('span').text(+like_count-1);
    ...
    if(requirement_id && project_id){            
        $.ajax({
            ...
            success:function(data){      
                ...
                if(data=='success')
                {                    
                var like_count = $('#'+my_id).parent().find('.like').find('span').text();
                var dislike_count = $('#'+my_id).parent().find('.dislike').find('span').text();
		var current_class = $('#'+my_id).attr('class');  
                        $('#'+my_id).css('display','none');
                        $('#'+my_id).parent().find('.like').css('display','inline-block');
                        $('#'+my_id).parent().find('.dislike').css('display','inline-block');
                        
                        if(current_class == 'after_like'){
                        	$('#'+my_id).parent().find('.after_like').find('span').text(+like_count-1);
                        	$('#'+my_id).parent().find('.like').find('span').text(+like_count-1);
                        } else if(current_class == 'after_dislike'){
	                        $('#'+my_id).parent().find('.dislike').find('span').text(+dislike_count-1);
	                        $('#'+my_id).parent().find('.after_dislike').find('span').text(+dislike_count-1);
                        }
                }
                else
                {
